"""
Multi-MNIST (4 digits) — Test-Time Compositional Generalization with Cobweb (PoE)
=================================================================================
...
multiplies them into a Product-of-Experts.  Here each image is a row of four MNIST digits
(slots 0,1,2,3 left→right), generated from FOUR independent factors — the digit in each slot —
with NO rotation/scale nuisance.  We hold out novel 4-digit combinations and test whether the
PoE composes them slot-by-slot.  Canvas is 28×112 grayscale replicated to 3 channels (D=9408)
so all the RGB / Inception / CLIP plumbing is unchanged.
...
Three steps:
  ...
  1. CONCEPTS = Cobweb nodes.  Each node n is a diagonal Gaussian N(m_n, σ²_n) with
     ...
     concepts (gross layout), deep nodes are specific ones (a particular 4-digit canvas) — the
     paper's coarse-to-fine mode hierarchy, for free.
  ...
  2. SELECT concepts with the paper's greedy argmax (Eq. 9) over a candidate POOL gathered from
     ...
     pixels still unexplained — so a concept sharp in one slot and one sharp in another are
     found on different branches.  HOW MANY concepts: fixed K, or the leftover-image cutoff —
     stop once μ_T explains ≥99% of the query.
...
Benchmark: 28×112 (gray→3ch), 4 slots × 10 digits.  ~500 seen 4-digit combinations,
12 held-out OOD combinations (every (slot, digit) individually seen).  Baselines: Top-1 /
Top-3 nearest seen-combination retrieval.  Metrics: FID, CLIP, k-NN P/R/F1 vs Faithfulness
(queries) and Generalization sets; joint = all 4 slot digits recovered.
"""
...
OUT_DIR  = os.path.join(HERE, "multimnist_output"); os.makedirs(OUT_DIR, exist_ok=True)
...
K_MAX    = 8       # safety cap on concepts per query (room for up to 4 slots + extras)
...
# ── Layout: a row of four 28×28 digits; factors = digit at slot 0,1,2,3 (left→right) ──
CELL = 28; ROWS, COLS = 1, 4; NPOS = ROWS * COLS             # four digits in a row
H, W = CELL * ROWS, CELL * COLS; D = H * W * 3               # 28×112, gray replicated to 3 channels
POS_NAME = ["s0", "s1", "s2", "s3"]                          # slot 0..3, left to right
_origins = [(r * CELL, c * CELL) for r in range(ROWS) for c in range(COLS)]   # left→right

# ── Load MNIST (train canvases ← MNIST train; OOD canvases ← MNIST test) ────────
...
def render(quad_imgs):
    """Place four 28×28 gray digits left→right into the 28×112 row and replicate to 3 channels."""
    c = np.zeros((H, W), np.float32)
    for (r0, c0), g in zip(_origins, quad_imgs):
        c[r0:r0+CELL, c0:c0+CELL] = g
    return np.repeat(c[:, :, None], 3, axis=2).reshape(-1)

# ── Compositional split: sample seen 4-digit combos; hold out novel OOD combos ──
N_SEEN, N_OOD = 500, 12                                       # number of distinct digit-combinations
def _rand_combo(): return tuple(int(d) for d in RNG.integers(0, 10, NPOS))
def make_split():
    seen = set()
    while len(seen) < N_SEEN: seen.add(_rand_combo())
    seen = list(seen)
    seen_pd = {(p, t[p]) for t in seen for p in range(NPOS)}  # (slot, digit) pairs the model has seen
    ood = set()
    while len(ood) < N_OOD:
        t = _rand_combo()
        if t in seen or t in ood: continue
        if all((p, t[p]) in seen_pd for p in range(NPOS)):    # every factor value individually seen
            ood.add(t)
    return seen, list(ood)
SEEN_SLOTS, OOD_SLOTS = make_split()
print(f"Split: {len(SEEN_SLOTS)} seen / {len(OOD_SLOTS)} OOD combinations")

N_PER_SEEN, N_QUERY, N_GEN = 6, 16, 16      # exemplars per combo (different handwriting per slot)
def _draw(by, d): pool = by[d]; return int(pool[RNG.integers(len(pool))])
def _canvas(by, gray, t): return render([gray[_draw(by, t[p])] for p in range(NPOS)])

X_train, slot_tr, lab_tr = [], [], []
for t in SEEN_SLOTS:
    for _ in range(N_PER_SEEN):
        X_train.append(_canvas(_tr_by, GR_TR, t)); slot_tr.append(t); lab_tr.append(t)
X_train = np.asarray(X_train, np.float32); lab_tr = np.asarray(lab_tr); dig_tr = lab_tr[:, 0]   # dig_tr = leftmost (slot-0) digit
ood_queries, ood_genset = {}, {}
for t in OOD_SLOTS:
    ims = np.asarray([_canvas(_te_by, GR_TE, t) for _ in range(N_QUERY + N_GEN)], np.float32)
    ood_queries[t] = ims[:N_QUERY]; ood_genset[t] = ims[N_QUERY:]
print(f"  train {X_train.shape}; OOD {len(OOD_SLOTS)}×{N_QUERY} queries + {N_GEN} gen imgs")

# combos disjoint by construction; train↔test MNIST exemplar split → no exemplar leakage
assert set(SEEN_SLOTS).isdisjoint(set(OOD_SLOTS))
assert all((p, t[p]) in {(q, s[q]) for s in SEEN_SLOTS for q in range(NPOS)} for t in OOD_SLOTS for p in range(NPOS))
print("  ✓ verified: seen/OOD combos disjoint, every (slot,digit) seen, train vs test exemplars")
...
# vary within that region (e.g. different handwriting of the same slot digit) while their content
...
def _chw(Xf): return torch.tensor(Xf.reshape(-1, H, W, 3), dtype=torch.float32).permute(0, 3, 1, 2)
@torch.no_grad()
...
# per-slot digit classifiers (kept only for an interpretable JOINT-accuracy context column)
print("Training per-slot digit classifiers …")
CLF = [LinearSVC(max_iter=5000).fit(X_train, lab_tr[:, p]) for p in range(NPOS)]
...
def evaluate(fn):
    ...
    for s in OOD_SLOTS:
        ...
        ok = np.ones(len(G), bool)                                # joint = all 4 slot digits recovered
        for p in range(NPOS): ok &= (CLF[p].predict(G) == s[p])
        acc["joint"].append(100 * ok.mean())
...
fig.suptitle(f"Multi-MNIST OOD — metrics by method (mean ± SE over {len(OOD_SLOTS)} OOD combos)", fontsize=11)
...
def show(ax, vec, title=None, ylabel=None):
    ax.imshow(np.clip(vec.reshape(H, W, 3), 0, 1)); ax.set_xticks([]); ax.set_yticks([])
...
def _slabel(t): return f"{t[0]} {t[1]} {t[2]} {t[3]}"          # the four slot digits as a label

# (1) method comparison
cols = list(METHODS)
fig, axes = plt.subplots(N_SHOW, 1 + len(cols), figsize=((1+len(cols)) * 2.4, N_SHOW * 0.95))
fig.suptitle("Multi-MNIST OOD composition — held-out 4-digit query reconstructed by each method", fontsize=11)
for r, t in enumerate(show_slots):
    x = ood_queries[t][0]
    show(axes[r, 0], x, "query" if r == 0 else None, _slabel(t))
...
#      descendant leaf per concept, so variety = handwriting/style while the combo stays fixed.
N_SAMP = 6
fig, axes = plt.subplots(N_SHOW, 2 + N_SAMP, figsize=((2 + N_SAMP) * 2.4, N_SHOW * 0.95), squeeze=False)
fig.suptitle(f"Multi-MNIST OOD — Cobweb-prior samples (scheme A): query · μ_T · {N_SAMP} clean leaf-resampled draws", fontsize=11)
for r, t in enumerate(show_slots):
    x = ood_queries[t][0]
    show(axes[r, 0], x, "query" if r == 0 else None, _slabel(t))
...
def _gather(selector):
    rows = []
    for t in show_slots:
        x = ood_queries[t][0]; mu, nodes, w, gains = poe_compose(x, selector)
        rows.append((x, t, mu, nodes, w, gains))
    ncol = max((len(r[3]) for r in rows), default=1)
    return rows, ncol

def render_concepts(selector, out_path, title):
    rows, ncol = _gather(selector)
    fig, axes = plt.subplots(N_SHOW, 2 + ncol, figsize=((2 + ncol) * 2.4, N_SHOW * 0.95), squeeze=False)
    fig.suptitle(title, fontsize=10)
    for r, (x, t, mu, nodes, w, gains) in enumerate(rows):
...
#      (reshaped to the canvas grid, averaged over channels); %label = share of coverage gain ΣΔ.
...
def render_hierarchy(root_node, out_path, title, max_depth=3, top_children=HIER_TOP):
    ...
    de(root_node, 0)
    def dn(n, d):
        x0, _ = pos[id(n)]; img = np.clip(np.asarray(n.mean, np.float32).reshape(H, W, 3), 0, 1)
        cnt = NODE_COUNTS.get(id(n))
        if cnt is not None and cnt.sum() > 0:
            dom = int(cnt.argmax()); color = _cmap(dom); lab = f"n={int(n.count)}\ns0 {dom}·{100*cnt[dom]/cnt.sum():.0f}%"
        else:
            color, lab = "black", f"n={int(n.count)}"
        ax.add_artist(AnnotationBbox(OffsetImage(img, zoom=0.95), (x0, d), frameon=True, bboxprops=dict(edgecolor=color, lw=2.5)))
    ...
    ax.legend(handles=[plt.Rectangle((0, 0), 1, 1, color=_cmap(i), label=str(i)) for i in range(10)],
              title="dominant slot-0 (leftmost) digit", loc="lower center", ncol=10, fontsize=8, title_fontsize=9, bbox_to_anchor=(0.5, -0.04))
...
for i, node in enumerate(level3):
    ...
    render_hierarchy(node, os.path.join(SUB_DIR, f"subtree_{i:02d}_s0d{dom}.png"),
                     f"Level-3 subtree (3 levels deep) — root n={int(node.count)}, dominant slot-0 digit {dom}", 3)
    _nsub += 1
...
NP = H * W   # spatial pixels per channel (28×112 = 3136)
...
for s in OOD_SLOTS:
    for x in ood_queries[s]:
        ...
        for k in range(len(w)):
            MAPS.append(w[k].reshape(H, W, 3).mean(2).reshape(NP))   # channel-avg donation heatmap
...
for ax, depth in [(axes[0], 1), (axes[1], 2), (axes[2], 3), (axes[3], 4)]:
    ...
    ax.imshow(lab.reshape(H, W), cmap=_PAL, vmin=0, vmax=len(_PAL.colors) - 1)
...
ax.imshow(_bg_first(bl_lab).reshape(H, W), cmap=_PAL, vmin=0, vmax=len(_PAL.colors) - 1)
ax.set_xticks([]); ax.set_yticks([])
...
def render_region_hierarchy(root, out_path, title, max_depth=4, top_children=6, root_idxs=None, basic_fps=None):
    ...
    def dn(n, d):
        ...
        img = _cw(np.clip((prof.reshape(H, W) + 1) / 2, 0, 1))[..., :3]   # continuous, true value
        _bl = basic_fps is not None and _fp(n) in basic_fps          # basic-level node?
        _ec, _lw = ("red", 3.0) if _bl else ("black", 1.2)
        ax.add_artist(AnnotationBbox(OffsetImage(img, zoom=0.95), (x0, d), frameon=True, bboxprops=dict(edgecolor=_ec, lw=_lw)))
