"""
Single-seed training + evaluation for TRELLIS.

Trains incrementally on the supplied CFG-derived corpus, snapshotting the
paper's two dependent variables every ``EVAL_EVERY`` sentences:

    * parse accuracy (Langley & Stromsten omission + commission on brackets)
    * generation grammaticality (commission-avoidance rate on 500 samples)

Writes ``learning_curves.csv`` and ``learning_curves.png`` to ``out_dir``.
Called once per (variant, seed) by ``experiment_harness.py``; not intended
