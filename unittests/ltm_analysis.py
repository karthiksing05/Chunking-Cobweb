...
for i, doc in enumerate(document):
    ...
    if i == 50:
        parser.visualize_ltm("unittests/ltm_analysis/half_ltm")
        parser.save_state("unittests/ltm_analysis/halfway_test")

# calling the basic-level tree draw method!
parser.cobweb_drawer.save_basic_level_subtrees(parser.ltm_hierarchy.root, "unittests/ltm_analysis/basic_level_subtrees", debug=True)
parser.cobweb_drawer.save_level_subtrees(parser.ltm_hierarchy.root, "unittests/ltm_analysis/level_2_subtrees", level=2)
parser.cobweb_drawer.save_level_subtrees(parser.ltm_hierarchy.root, "unittests/ltm_analysis/level_3_subtrees", level=3)
