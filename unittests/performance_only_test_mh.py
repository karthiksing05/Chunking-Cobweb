"""
Performance-only script for a saved WEBSTER LTM.

This script loads a previously saved LTM (from
`unittests/gen_learn_test_mh/final_ltm_data` or another saved dir) and runs
generation-only procedures plus parsing evaluations of the generated sentences
and a set of fake sentences. Results (parse-tree visuals and text outputs)
are written to `unittests/performance_only_test_mh`.
"""
...
if not os.path.exists(LOAD_LTM_DIR):
    print(f"Saved LTM not found at {LOAD_LTM_DIR}. Aborting performance-only run.")
    raise SystemExit(1)
...
# Generate fresh random sentences using the grammar. These will be used as inputs
# for masked completion/prediction tests and parsing evaluation.
random.seed(42)
num_sentences = 100
