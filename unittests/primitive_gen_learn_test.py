"""
General Learning Test - to confirm the logic of learning is completely functional!

THIS TEST HAS PRIMITIVES!!! We're going to arbitrarily build the primitive hierarchy first
and then have "epochs" of levels just to confirm that structures are being built normally.
"""

from util.cfg import generate, TEST_GRAMMAR1, TEST_CORPUS1
from parse import LanguageChunkingParser
import shutil
import os

if os.path.isdir("unittests/primitive_gen_learn_test"):
    shutil.rmtree("unittests/primitive_gen_learn_test")

# Creating and printing toy sentences
CONTEXT_LENGTH = 3

num_sentences = 100
document = []

for _ in range(num_sentences):
    sentence = generate("S", TEST_GRAMMAR1)
    document.append(sentence)

# Setting up the parser
parser = LanguageChunkingParser(TEST_CORPUS1, context_length=CONTEXT_LENGTH, merge_split=True)

train_size = 0.90

train_documents = document[:int(len(document) * train_size)]
test_documents = document[int(len(document) * train_size):]

# Iterate through training documents and parse them one at a time, saving every 10th parse tree to file
for i, doc in enumerate(train_documents):
    threshold = (0 if i < 90 else -15) # should never trigger atp
    print("Threshold:", threshold)
    parse_trees = parser.parse_input([doc], end_behavior=threshold, debug=True)
    parse_tree = parse_trees[0]

    parser.add_parse_tree(parse_tree, debug=False)

    if i < 5:
        parser.visualize_ltm(f"unittests/primitive_gen_learn_test/ltms/cobweb_ltm{i}")

    if i % 5 == 0:
        parse_tree.visualize(f"unittests/primitive_gen_learn_test/train_trees/train_parse_tree{i}")

        if i < 21:
            parser.visualize_ltm(f"unittests/primitive_gen_learn_test/ltms/cobweb_ltm{i}", max_depth=4)

parser.visualize_ltm("unittests/primitive_gen_learn_test/final_ltm", max_depth=4)

# perhaps visualize the last ten parse trees based on new inputs
for i, test in enumerate(test_documents):
    parse_tree = parser.parse_input([test], end_behavior="converge", debug=False)[0]
    parse_tree.visualize(f"unittests/primitive_gen_learn_test/test_trees/test_parse_tree{i}")
    print(f"Created parse tree {i} for sentence, \"{test}\"")

SAVE_DIR = "unittests/primitive_gen_learn_test/final_ltm_data"
parser.save_state(SAVE_DIR)
print(f"Saved LTM to \"{SAVE_DIR}\"!")