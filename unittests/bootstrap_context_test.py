...
Uses the same WEBSTER setup as primitives_only_test_mh.py but runs
bootstrap pre-training instead of sentence parsing.
"""

from util.cfg import generate, TEST_GRAMMAR_LARGE, TEST_CORPUS_LARGE
...
for _ in range(num_sentences):
    sentence = generate("S", TEST_GRAMMAR_LARGE)
...
# Bootstrap context hierarchy (accumulated distribution only)
print("Running bootstrap_context_hierarchy ...")
webster.bootstrap_context_hierarchy(
    document,
    debug=True,
)

# ── Print word clusters at a configurable depth ─────────────────────────
PRINT_DEPTH = 3  # set to desired depth (1 = root's children, 2 = grandchildren, etc.)
...
def _get_leaf_words(node, id_to_value):
    ...
    if not node.children:
        # leaf: attr -1 holds {word_id: count}
...
print(f"\n=== Word clusters at depth {PRINT_DEPTH} ({len(nodes_at_depth)} nodes) ===")
