"""
confs/acs-26 / Grammar LARGE (TEST_GRAMMAR_LARGE)
=================================================

Same as ``run_grammar_med.py`` but for ``TEST_GRAMMAR_LARGE`` — adds
RelClause modifiers and a RelPro POS class on top of TEST_GRAMMAR_MED.
Both the hollow_learn pipeline and the learning-curves test are run
with **5 seeds** and aggregated as mean ± std.

Outputs (all under ``confs/acs-26/grammar_large/``):
...
OUT_BASE   = os.path.join(_HERE, "grammar_large")
...
plot_learning_curves_with_band(
    ...
    agg=lc_agg,
    title="TEST_GRAMMAR_LARGE — learning curves",
    n_seeds=len(SEEDS),
)
